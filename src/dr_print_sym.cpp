...
void Sym::print( const char *const description, const app_pc addr ) {
	...
	std::unique_ptr<module_data_t, void ( * )( module_data_t * )> data_encapsulated(
	    data, dr_free_module_data );
	...
	switch ( symres ) {
		...
		default:
			Utilities::log_error( "drsym_lookup_address() failed for ", (void *) addr,
			                      "with error number: ", symres );
			return;
